...
const Weather = () => {
  ...
  const handleWeather = () => {
    fetch(
      ` https://api.weatherapi.com/v1/current.json?key=${WEATHER_API_KEY}&q=Kolkata&aqi=no`
    )
      .then((res) => res.json())
      .then((data) =>
        setWeather({
      ...
      {loading ? (
        ...
        <>
          <div className="weather-top">
            <p className="date">
              {weather.current.last_updated.substring(0, 10)}
            </p>
            <p className="time">
              {weather.current.last_updated.substring(11, 17)}
            </p>
          </div>
          <div className="weather-bottom">
            ...
            <div className="w-box w-box3">
              <p className="w-box3-txt-1">
                <span className="w-box3-span-1">
                  {" "}
                  <img src={img2} alt="img" width="30px" />
                </span>
                {weather.current.wind_kph}km/h <br />{" "}
              </p>
              <p className="w-box3-txt-2">
                <span className="w-box3-svg">
                  {" "}
                  <svg
                    width="20"
                    height="38"
